...
class Project(Tk):
    def __init__(self):
        ...
        self.myImage        = ImageTk.PhotoImage(Image.open("images\\dasmon_logo.png").resize([85,85]))
        self.imageLabel     = Label(self.title_frame, image=self.myImage, bg=self.colors["panel_1"],)
        self.imageLabel.pack(side=LEFT)
        self.label_title    = Label(self.title_frame, text="Discrete Structure Assignmnet", bg=self.colors["panel_1"], fg=self.colors["text_1"], font=self.fonts["title"], padx=20, pady=2)
    ...
    def switch_frame(self, frame_class):
        ...
        if frame_class is startup_screen:
            if self.startup_frame is None:
                ...
                self.startup_frame.pack(side=TOP, fill=NONE, anchor=CENTER, expand=True)
                self.startup_frame.tkraise()
            self.label_subtitle.config(text="Question: Given the adjacency matrix of an undirected simple graph, determine whether the graph is a tree.")
            self.startup_frame.update()
            if self.done == 0:
                self.copyrightFrame = Frame(self, bg=self.colors["title_bg"])        
                self.lbl = Label(self.copyrightFrame, text="Made by Anurag, Fazal and Kartikya", font=self.fonts["bold"], pady=10, bg=self.colors["panel_1"], fg=self.colors["text_1"]) 
        ...
        if frame_class is settings_screen:
            if self.settings_frame is None:
                ...
                #self.settings_frame.disable_fields()
                for widget in self.settings_frame.winfo_children():
                    widget.destroy()
                self.settings_frame = settings_screen(self)
                self.settings_frame.pack(side=TOP, fill=NONE, anchor=CENTER)
                #self.settings_frame.update_table_list()
                self.settings_frame.tkraise()
    ...
    '''def init_settings(self):
        ...
        self.prog_settings = np.load('resources\\settings_data.npy', allow_pickle='TRUE').item()

    def save_settings(self):
        """Read the setting from file"""
        np.save('resources\\settings_data.npy', self.prog_settings)'''
